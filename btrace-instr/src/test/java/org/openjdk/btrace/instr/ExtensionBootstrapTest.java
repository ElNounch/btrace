...
class ExtensionBootstrapTest {
  ...
  static void setupAll() throws Throwable {
    ExtensionRepository repo = mock(ExtensionRepository.class);
    ExtensionEntry entry = mock(ExtensionEntry.class);
    when(entry.getClassLoader(Mockito.any(ClassLoader.class)))
        .thenReturn(ExtensionBootstrapTest.class.getClassLoader());
    when(repo.getExtensionById(Mockito.anyString())).thenReturn(entry);
  ...
  void bootstrapInvokeExistingMethod(String methodName) throws Throwable {
    ...
    assertEquals(1, TestExtension.getInvocations(methodName));
  }
